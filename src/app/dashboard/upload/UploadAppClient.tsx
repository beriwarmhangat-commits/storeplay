...
export default function UploadAppClient({ message, type }: { message?: string, type?: string }) {
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      // Navigasi biasanya menghandle hilangnya loading, 
      // tapi kita set false jika ada error atau tetap di halaman
      setLoading(false)
    }
  }
  ...
  return (
    <div className="container" style={{ padding: '3rem 1.5rem', paddingBottom: '6rem', maxWidth: '800px' }}>
      {loading && <LoadingOverlay message="Sedang mendaftarkan aplikasi Anda..." />}
      ...
      <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem' }}>Detail awal untuk aplikasi Anda. Admin akan meninjau aplikasi sebelum dipublikasikan.</p>
      ...
      <form action={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', backgroundColor: 'var(--bg-secondary)', padding: '2rem', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)' }}>
